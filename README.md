...
2. 「設定」タブで Box の `クライアントID`・`クライアントシークレット`・`Box API 検索件数`・`サイドカー待ち受けポート`・`MCP サーバーURL` 等を入力し「保存」を押してください。これらは OS の Keyring に保存されます。ポートは次回起動時に反映され、環境変数 `KASUGAI_BOX_PORT` で上書き可能です。
